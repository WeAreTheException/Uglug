...
func get_attack() -> int:
	return max(base_attack + _total(StatName.ATTACK), 0)

func get_health() -> int:
	return max(get_max_health() - damage_taken, 0)

func get_max_health() -> int:
	return max(base_health + _total(StatName.HEALTH), 0)

func get_cost() -> int:
	return max(base_cost + _total(StatName.COST), 0)

func get_worth() -> int:
	return max(base_worth + _total(StatName.WORTH), 0)

func add_modifier(modifier: StatModifier) -> void:
	if modifier == null:
		return
	modifiers.append(modifier)
	_emit_buff_signal(modifier)
	_emit_all_changed()
...
func remove_modifiers_from_source(source: Object) -> void:
	for i in range(modifiers.size() - 1, -1, -1):
		if modifiers[i] != null and modifiers[i].source == source:
...
func take_damage(amount: int) -> void:
	if amount <= 0:
		return
	damage_taken = min(damage_taken + amount, get_max_health())
...
func _total(stat_name: String) -> int:
	return CardStatsCalculator.get_modifier_total(modifiers, stat_name)

func _clamp_damage_taken() -> void:
	damage_taken = clampi(damage_taken, 0, get_max_health())

func _emit_buff_signal(modifier: StatModifier) -> void:
