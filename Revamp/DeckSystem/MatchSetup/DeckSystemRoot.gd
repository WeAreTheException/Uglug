...
@export_group("Manual Draw")
@export var worker_draw_button: BaseButton
@export var warrior_draw_button: BaseButton
@export var manual_draw_handler: MatchManualDrawHandler

@export_group("Starting Hand")
...
func build_match_decks() -> void:
	...
	var p1_draw: Array[CardData] = _to_card_data_array(
		result["p1_draw_pile"]
	)

	var p2_draw: Array[CardData] = _to_card_data_array(
		result["p2_draw_pile"]
	)

	var p1_start: Array[CardData] = _to_card_data_array(
		result["p1_starting_hand"]
	)

	var p2_start: Array[CardData] = _to_card_data_array(
		result["p2_starting_hand"]
	)
...
func apply_match_setup_payload(
	payload: Dictionary
) -> void:
	_setup_hand_contexts()
...
func get_card_data(card_id: String) -> CardData:
	...
	if clean_id == "":
		print(
			"DeckSystemRoot card lookup failed: "
			+ "empty card_id"
		)
		return null
...
func apply_confirmed_draw(
	...
	if (
		pop_local_pile
		and pile_type == DRAW_PILE_WARRIOR
	):
		_pop_and_check_local_draw_pile(
			owner,
			card_id,
			runtime_id
		)
	...
	if hand == null:
		print(
			"confirmed draw blocked: hand missing"
		)
		return null

	if card_data == null:
		print(
			"confirmed draw blocked: "
			+ "card data missing ",
			card_id
		)
		return null
	...
	var card := (
		hand.spawn_card_from_effect_with_runtime_id(
			card_data,
			runtime_id,
			true
		)
	...
	if card != null:
		_apply_inherited_mutations_to_card(
			card,
...
func _apply_inherited_mutations_to_card(
	...
	for mutation_id in inherit_mutation_ids:
		var mutation := _find_mutation_by_id(
			mutation_id
		)

		if mutation == null:
			print(
				"INHERIT MUTATION FAILED: ",
				mutation_id
			)
			continue

		card.mutations.add_buff_mutation(
			mutation
		)


func draw_warrior_for_owner(
	owner: SlotRow.SlotOwner
) -> void:
	var entry := pop_draw_entry_for_owner(
		owner,
		DRAW_PILE_WARRIOR
	)
...
func _apply_inherited_mutations_to_entry(
	...
	entry["inherited_mutation_ids"] = (
		inherit_mutation_ids.duplicate()
	)


func find_card_anywhere(
	runtime_id: String
) -> CardRoot:
...
func _deal_starting_hands_animated_from_payload() -> void:
	await get_tree().create_timer(
		starting_hand_start_delay
	).timeout

	var p1_entries: Array = (
		last_setup_payload.get(
			"p1_starting_hand",
			[]
		)
	...
	for i: int in range(max_count):
		...
		await get_tree().create_timer(
			starting_hand_card_delay
		).timeout

	starting_hands_dealt.emit()


func _spawn_payload_hand(
	hand: PlayerHandRoot,
	entries: Array
) -> void:
	if hand == null:
...
func _spawn_payload_card(
	...
	var card_id: String = entry.get(
		"card_id",
		""
	)

	var runtime_id: String = entry.get(
		"runtime_id",
		""
	)
	...
	return hand.spawn_card_with_runtime_id(
		card_data,
		runtime_id
	)
...
func _pop_and_check_local_draw_pile(
	...
	var draw_pile := get_draw_pile_for_owner(
		owner
	)
	...
	if local_entry.is_empty():
		print(
			"confirmed draw warning: "
			+ "local draw pile empty"
		)
		return
...
func _setup_manual_draw() -> void:
	if manual_draw_handler == null:
		return

	manual_draw_handler.setup(
		worker_draw_button,
		warrior_draw_button,
		self
	)
...
func _setup_hand_contexts() -> void:
	if player_one_hand != null:
		player_one_hand.setup_deck_system_context(
			self
		)

	if player_two_hand != null:
		player_two_hand.setup_deck_system_context(
			self
		)


func _get_hand_for_card_owner(
	source_card: CardRoot
) -> PlayerHandRoot:
	var owner: SlotRow.SlotOwner = (
		_get_owner_for_card(source_card)
	)

	return get_hand_for_owner(owner)


func _get_owner_for_card(
	source_card: CardRoot
) -> SlotRow.SlotOwner:
	if source_card == null:
	...
	if source_card.slots_root != null:
		...
		if slot != null:
			return (
				source_card
				.slots_root
				.get_owner_of_slot(slot)
			)

	if (
		player_two_hand != null
		and player_two_hand.has_card(source_card)
	):
		return SlotRow.SlotOwner.OPPONENT

	if (
		player_one_hand != null
		and player_one_hand.has_card(source_card)
	):
		return SlotRow.SlotOwner.PLAYER
...
func _find_card_in_hand(
	...
	return hand.find_card_by_runtime_id(
		runtime_id
	)
...
func _register_card_data_array(
...
func _register_card_data(
	...
	var safe_id := (
		card_data.get_safe_card_id().strip_edges()
	)
...
func _apply_payload_debug() -> void:
	...
	print(
		"PAYLOAD APPLY DEBUG: "
		+ "applying last setup payload"
	)

	apply_match_setup_payload(
		last_setup_payload
	)


func _get_inheritable_mutation_ids(
	source_card: CardRoot
) -> Array[String]:
...
func _find_mutation_by_id(
	...
	return buff_database.get_mutation_by_id(
		clean_id
	)
