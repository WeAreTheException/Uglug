...
func request_preview_slot(slot: Slot) -> void:
	...
	if placement_preview != null:
		placement_preview.show_preview(placement_state.active_card, slot, get_preview_move_time())
...
func is_valid_placement_slot(slot: Slot) -> bool:
	if placement_state == null:
		return false
	return slot_validator.is_valid_slot(slots_root, slot, placement_state.active_owner)

func get_left_to_right(owner: SlotRow.SlotOwner) -> bool:
	if config == null:
		return owner == SlotRow.SlotOwner.PLAYER
	return config.opponent_left_to_right if owner == SlotRow.SlotOwner.OPPONENT else config.player_left_to_right
