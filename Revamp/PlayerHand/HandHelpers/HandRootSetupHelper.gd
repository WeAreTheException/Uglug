...
func _setup_spawner(root: PlayerHandRoot, cb: HandRootCallbacksHelper) -> void:
	...
	if spawner == null:
		return
	spawner.configure(root.card_scene, root.starting_cards, root.hand_card_layer, root.max_hand_size, root.minimum_hand_size)
...
func _setup_interaction(root: PlayerHandRoot, cb: HandRootCallbacksHelper) -> void:
	...
	if interaction == null:
		return
	interaction.setup(root.card_spawner, root.hand_layout, root.hand_card_layer, root.drag_layer, root.prime_location)
...
func _setup_sacrifice(root: PlayerHandRoot, cb: HandRootCallbacksHelper) -> void:
	...
	_connect(root.sacrifice_selection.hand_sacrifice_selection_changed, Callable(cb, "on_sacrifice_selection_changed"))
...
func _setup_state_machine(root: PlayerHandRoot, cb: HandRootCallbacksHelper) -> void:
	if root.state_machine == null:
		return
	root.state_machine.setup(root.hand_layout, root.interaction_root, root.sort_controller, root.sacrifice_selection)
...
func _connect_buttons(root: PlayerHandRoot) -> void:
	_connect_named(root.prime_button_root, "prime_pressed", Callable(root, "request_prime_toggle"))
	_connect_named(root.sort_buttons_root, "sort_by_cost_pressed", Callable(root, "request_sort_by_cost"))
	_connect_named(root.sort_buttons_root, "sort_by_mutation_pressed", Callable(root, "request_sort_by_mutation_count"))
	_connect_named(root.sacrifice_button_root, "sacrifice_pressed", Callable(root, "request_sacrifice"))

func _connect_named(source: Object, signal_name: StringName, target: Callable) -> void:
	if source != null and source.has_signal(signal_name):
		_connect(source.get(signal_name), target)
