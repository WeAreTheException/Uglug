...
func setup(source_card_spawner: Hand_CardSpawner) -> void:
	...
	if card_spawner != null and not card_spawner.card_removed.is_connected(_on_card_removed):
...
func deselect_card(card: CardRoot) -> void:
	if card == null or not selected_cards.has(card):
		return
...
func _can_select(card: CardRoot) -> bool:
	return (
		card != null
		and card != primed_card
		and not selected_cards.has(card)
		and card_spawner != null
