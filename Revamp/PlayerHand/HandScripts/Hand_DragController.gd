...
func forget_card(card: CardRoot) -> void:
	...
	var was_dragging := state.dragged_card == card

	state.reset()

	# Prevents the rest of the hand from remaining disabled if the
	# dragged card is removed before a normal mouse release.
	if was_dragging and interaction_root != null:
		interaction_root.set_card_hover_input_enabled(true)
		interaction_root.refresh_hover_focus()
...
func _try_start_drag() -> void:
	var distance := (
		...
		.distance_to(state.press_mouse_position)
	)
	...
	# Other cards stop detecting hover here, only after the movement
	# has crossed the actual drag threshold.
	interaction_root.begin_drag(state.dragged_card)
	state.dragged_card.z_index = dragged_z_index


func _update_drag() -> void:
	mover.apply_drag_position(
		state.dragged_card,
		state.drag_offset,
		dragged_z_index
	)
...
func _cancel_previous_hold(new_card: CardRoot) -> void:
	...
	if state.held_card == new_card:
		return

	var previous_card := state.held_card
	var was_dragging := state.dragged_card != null

	_set_card_drag_state(
		previous_card,
		false,
		previous_card.z_index
	)

	if was_dragging:
		interaction_root.finish_drag(previous_card)
...
func _cancel_drag_state() -> void:
	if state.dragged_card != null:
		_set_card_drag_state(
			state.dragged_card,
			false,
			state.dragged_card.z_index
		)

		interaction_root.finish_drag(state.dragged_card)

	elif state.held_card != null:
		_set_card_drag_state(
			state.held_card,
			false,
			state.held_card.z_index
		)

	state.reset()


func _set_card_drag_state(
	card: CardRoot,
	value: bool,
	z_value: int
) -> void:
